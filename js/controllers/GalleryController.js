            ...
            if (this._index === null){
                $.ajax({
                    ...
                    success: function(data) {
                        ws._data = data;
                        ws._photos = new SinglePageApp.Collections.PhotoCollection(data);
                        ws._index = new SinglePageApp.Views.IndexView({
                            model: ws._photos
                            }); 
	...
	*/
	 ...
	 */
        hashsub:function(id){
            ...
            this._subphotos = cache.get('pc' + properindex) || cache.set('pc' + properindex, new SinglePageApp.Collections.PhotoCollection(this._data[properindex].subalbum));
            this._subalbums = cache.get('sv' + properindex) || cache.set('sv' + properindex, new SinglePageApp.Views.SubalbumView({
	 ...
	 */
        hashphoto: function(num, id){
            ...
            if(this._subphotos == undefined){
                this._subphotos = cache.get('pc' + num) || cache.set('pc' + num, new SinglePageApp.Collections.PhotoCollection(this._data[num].subalbum));
            }	
            this._subphotos.at(id)._view = new SinglePageApp.Views.PhotoView({
                model: this._subphotos.at(id)
                });
    ...
    $.ns("SinglePageApp.Controllers.GalleryController", Gallery);
})()
